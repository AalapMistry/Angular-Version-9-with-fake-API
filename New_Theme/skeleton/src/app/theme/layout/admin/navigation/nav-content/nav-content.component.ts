import { AfterViewInit, Component, ElementRef, EventEmitter, NgZone, OnInit, Output, ViewChild, OnDestroy } from '@angular/core';
...
export class NavContentComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(public nav: NavigationItem, private zone: NgZone, private location: Location,
    ...
    this.subscription = this.coreHelperService.getUserChangedInfo()
  ...
  ngOnInit() {
    ...
    if (!!localStorage.getItem("LoggedUserDetails")) {
      this.userDetails = <UserModel>JSON.parse(localStorage.getItem("LoggedUserDetails"));
    }
  }
